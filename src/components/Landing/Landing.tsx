...
const Section2 = forwardRef<HTMLDivElement, { calcOpacity: number }>((props, ref) => {
    ...
    return (
        <>
            ...
            >
                <SimpleGrid columns={[1, 3]} position={"sticky"} top={0} pointerEvents={calcOpacity < 0.01 ? "none" : "auto"}
                >
    ...
    const handleActivateLanding = () => {
        ...
        if (scrollContainerRef.current) {
            scrollContainerRef.current.scrollTo({
              top: scrollContainerRef.current.scrollTop - scrollContainerRef.current.clientHeight,
              behavior: 'smooth',
            });
        }
    };
