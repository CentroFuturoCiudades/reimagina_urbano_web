...
const useExtraLayers = () => {
    const condition = useSelector((state: RootState) => state.viewMode.project === "culiacan_sur");
    ...
    useAborterEffect(async (signal: any, isMounted: boolean) => {
        ...
        const filteredData = data.filter(
            (item: any) => item.properties.amenity === "Capilla" || item.properties.amenity === "Comedor"
        );
    ...
    const layer = new IconLayer({
        ...
        getColor: [50, 50, 50],
        iconAtlas: "https://images.vexels.com/content/155419/preview/thick-christian-cross-icon-032999.png",
        getPosition: (d: any) => d.geometry.coordinates,
        getIcon: (d: any) => "marker",
        data: polygons.filter((item: any) => item.properties.amenity === "Capilla"),
        sizeScale: 20,
    });
    const layer2 = new IconLayer({
        ...
        getPosition: (d: any) => d.geometry.coordinates,
        getIcon: (d: any) => "marker",
        data: polygons.filter((item: any) => item.properties.amenity === "Comedor"),
        sizeScale: 30,
    });
    return [layer, layer2];
};
...
const Layers = () => {
    ...
    useAborterEffect(
        async (signal: any, isMounted: boolean) => {
            ...
            if (metric !== "max_num_levels") metrics["max_num_levels"] = "max_num_levels";
            ...
            if (isMounted) {
                ...
                const ids: any[] = Array.from(new Set(dfData.map((x: any) => x[id])).values());
